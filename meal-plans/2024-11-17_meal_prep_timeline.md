# 🍳 MEAL PREP STRATEGIE
## Wochenplan: 17. bis 21. November 2024

**Prep-Tag:** Sonntag, 16. November (für Mo-Fr Woche)
**Gesamtzeit:** ~3,5-4 Stunden
**Schwierigkeitsgrad:** Mittel
**Portionen:** 5 Tage (15 Mahlzeiten)

---

## 📅 VORBEREITUNG (Samstag Abend, 15. November)
⏰ **10-15 Min**

### Hülsenfrüchte einweichen (falls getrocknet)
- [ ] **500g Kichererbsen** in große Schüssel, reichlich Wasser, über Nacht stehen lassen - *erhöht für Protein-Optimierung*
- [ ] 150g Cannellini-Bohnen einweichen
- [ ] **Tipp:** Mit 1 TL Natron einweichen (bessere Verdaulichkeit)

### Container vorbereiten
- [ ] 5 Schraubgläser für Overnight Oats (mind. 400ml)
- [ ] 10 Meal Prep Container für Mittag-/Abendessen
- [ ] Etiketten bereitlegen

### Einkaufsliste finalisieren
- [ ] Letzte Zutaten checken
- [ ] Gewürze kontrollieren

---

## ⏰ MEAL PREP TAG - ZEITPLAN ÜBERSICHT

```
09:00 - 09:45   Phase 1: Grundlagen (Getreide & Hülsenfrüchte)
09:45 - 10:45   Phase 2: Gemüse rösten & Overnight Oats
10:45 - 11:30   Phase 3: Spezial-Komponenten & Dressings
11:30 - 12:15   Phase 4: Portionieren & Beschriften
12:15 - 12:30   Aufräumen & Qualitätskontrolle
```

**Alternative:** Auf 2 Tage verteilen (Sa + So je 2h)

---

## 🥘 PHASE 1: GRUNDLAGEN KOCHEN
⏰ **09:00 - 09:45 (45 Min)**

### Parallel-Tasks (alles gleichzeitig!):

#### Task 1A: Quinoa kochen (20 Min aktiv + 15 Min passiv)
- [ ] 350g Quinoa in Sieb gründlich abspülen
- [ ] In Topf mit 700ml Wasser und Prise Salz
- [ ] Aufkochen, dann 15 Min bei niedriger Hitze köcheln
- [ ] 5 Min ruhen lassen, dann auflockern
- [ ] **Ergebnis:** ~1050g gekochte Quinoa

**Aufteilung nach dem Kochen:**
- 80g für Tag 1 Mittag
- 80g für Tag 1 Abend
- 80g für Tag 2 Mittag
- 80g für Tag 3 Abend
- 80g für Tag 4 Mittag
- 80g für Tag 4 Abend
- 80g für Tag 5 Mittag
- 80g für Tag 5 Abend
- Rest: Reserve

#### Task 1B: Linsen kochen (10 Min aktiv + 25 Min passiv)
- [ ] **360g grüne/braune Linsen** in Topf - *erhöht für Protein-Optimierung*
- [ ] 850ml Wasser hinzufügen
- [ ] Aufkochen, dann 20-25 Min köcheln (bissfest!)
- [ ] Abgießen, auskühlen lassen
- [ ] **Ergebnis:** ~860g gekochte Linsen

**Aufteilung (Protein-optimiert):**
- 180g für Tag 2 Abend (erhöht!)
- 120g für Tag 4 Abend
- 180g für Tag 5 Abend (erhöht!)
- Rest: Reserve oder einfrieren

#### Task 1C: Kichererbsen kochen (10 Min aktiv + 60 Min passiv)
- [ ] Eingeweichte **500g Kichererbsen** abgießen, abspülen - *erhöht*
- [ ] In großen Topf mit viel frischem Wasser
- [ ] Aufkochen, Schaum abschöpfen
- [ ] 60-90 Min köcheln bis weich
- [ ] **Alternative:** Drucktopf 20-25 Min
- [ ] **Ergebnis:** ~1250g gekochte Kichererbsen

**Aufteilung (Protein-optimiert):**
- 120g für Tag 1 Mittag (geröstet)
- 150g für Tag 2 Mittag (Curry)
- **300g für Tag 3 Abend** (geröstet, erhöht!)
- 150g für Tag 5 Mittag (Curry)
- Rest: Reserve oder einfrieren

#### Task 1D: Cannellini-Bohnen kochen (10 Min aktiv + 60 Min passiv)
- [ ] Eingeweichte Bohnen abgießen, abspülen
- [ ] In Topf mit frischem Wasser
- [ ] 60-75 Min köcheln bis weich
- [ ] **Ergebnis:** ~375g gekochte Bohnen

**Verwendung:**
- 120g für Tag 1 Abend (Salat)
- Rest: Reserve oder einfrieren

---

## 🥗 PHASE 2: GEMÜSE & FRÜHSTÜCK
⏰ **09:45 - 10:45 (60 Min)**

### Ofen-Phase (parallel nutzen!):

#### Task 2A: Gemüse vorbereiten (20 Min)

**Rotkohl** (reduziert):
- [ ] Rotkohl waschen, Strunk entfernen
- [ ] Verschiedene Zubereitungen:
  - **Fein hobeln für Salate:** 180g (Tag 1 Abend, mariniert für Wrap Tag 4)
  - **Fein schneiden für Curry:** 150g (Tag 2 Mittag)
  - **Wedges zum Rösten:** 150g (Tag 3 Abend)
- [ ] In beschriftete Behälter/Beutel

**Hokkaido-Kürbis:**
- [ ] In Würfel schneiden (~400g, Schale kann dranbleiben!)
- [ ] Für Tag 1 Mittag (100g) und Tag 5 Abend (120g)

**Lauch/Porree:**
- [ ] 150g in feine Ringe schneiden, gründlich waschen (für Tag 3 Miso-Suppe)

**Fenchel:**
- [ ] 80g in Scheiben schneiden (für Tag 4 Abend Buddha-Bowl)

**Feldsalat:**
- [ ] 60g waschen, in Beutel für Tag 5 Abend

#### Task 2B: Gemüse rösten - Blech 1 (10 Min Prep + 25 Min Ofen)
**Ofen auf 200°C vorheizen**

- [ ] 150g Rotkohl-Wedges (für Tag 3 Abend)
- [ ] 220g Hokkaido-Kürbis-Würfel (für Tag 1 & Tag 5)
- [ ] Mit 1 EL Olivenöl, Salz, Pfeffer, Kreuzkümmel marinieren
- [ ] Auf Backblech verteilen
- [ ] **Rösten:** 20-25 Min (Rotkohl bis Ränder karamellisieren, Kürbis weich)

#### Task 2C: Gemüse rösten - Blech 2 (10 Min Prep + 25 Min Ofen)
- [ ] 300g Karotten in Sticks schneiden
- [ ] 300g Rote Bete würfeln (Handschuhe!)
- [ ] 80g Fenchel in Scheiben
- [ ] Mit 1 EL Olivenöl, Salz, Gewürzen (Kreuzkümmel) mischen
- [ ] **Rösten:** 25-30 Min

#### Task 2D: Kichererbsen rösten - Blech 3 (5 Min Prep + 25 Min Ofen)
- [ ] **420g gekochte Kichererbsen** gut abtropfen und trockentupfen - *erhöht für Protein-Optimierung*
- [ ] Mit Gewürzmischung vermengen:
  - 2 TL Kreuzkümmel
  - 2 TL Paprikapulver
  - 2 TL Knoblauchpulver
  - 1,5 EL Olivenöl
  - Salz, Pfeffer
- [ ] Auf großem Blech verteilen (eventuell 2 Bleche nutzen für Knusprigkeit!)
- [ ] **Rösten:** 25-30 Min (knusprig!)
- [ ] **Verwendung:** 120g Tag 1 Mittag, 300g Tag 3 Abend

#### Task 2E: Gemüse dämpfen (während Ofen läuft)
- [ ] 250g Brokkoli in Röschen schneiden
- [ ] 8 Min dämpfen (bissfest)
- [ ] Sofort kalt abschrecken
- [ ] **Verwendung:** Tag 1 Mittag (80g)

#### Task 2F: Overnight Oats vorbereiten (15 Min)

**5 Gläser vorbereiten (Protein-optimiert):**

**Glas 1 - Tag 1 (Apfel-Zimt):**
- 30g Haferflocken, 1 EL Chia, 1 EL Leinsamen, **20g Protein** ✨
- 150ml Hafermilch, 1 EL Mandelmus
- Mix gut, verschließen

**Glas 2 - Tag 2 (Protein-Boost):**
- 30g Haferflocken, 1 EL Chia, 1 EL Leinsamen, **25g Protein** ✨
- 200ml Hafermilch, 1 EL Cashewmus
- 1 EL Hanfsamen zur Basis
- Mix gut, verschließen

**Glas 3 - Tag 3 (Schoko-Banane):**
- 30g Haferflocken, 1 EL Chia, 1 EL Leinsamen, **20g Protein** ✨
- 150ml Hafermilch, 1 EL Erdnussmus, 1 EL Kakao
- Mix gut, verschließen

**Glas 4 - Tag 4 (Quinoa-Bowl mit Protein-Boost):**
- [ ] Keine Overnight Oats - Quinoa-Frühstück am Morgen frisch
- [ ] **WICHTIG:** 25g Erbsenprotein-Pulver bereitstellen für Tag 4 Morgen ✨

**Glas 5 - Tag 5 (Beeren-Power):**
- 30g Haferflocken, 1 EL Chia, 1 EL Leinsamen, **20g Protein** ✨
- 150ml Hafermilch, 1 EL Mandelmus
- Mix gut, verschließen

- [ ] Alle Gläser beschriften und in Kühlschrank
- [ ] **Haltbarkeit:** 3 Tage (Mi neue Portion machen!)

---

## 🔪 PHASE 3: SPEZIAL-KOMPONENTEN
⏰ **10:45 - 11:30 (45 Min)**

### Task 3A: Dressings & Saucen (20 Min)

#### Tahini-Dressing (für ganze Woche):
- [ ] 4 EL Tahini
- [ ] 3 EL Zitronensaft (1,5 Zitronen)
- [ ] 1 Knoblauchzehe, gepresst
- [ ] 4 EL Wasser
- [ ] 1 TL Kreuzkümmel, Salz, Pfeffer
- [ ] Alles mixen, in Schraubglas
- [ ] **Haltbarkeit:** 5-7 Tage

#### Apfel-Balsamico-Vinaigrette:
- [ ] 3 EL Apfelessig
- [ ] 2 EL Balsamico
- [ ] 1 EL Ahornsirup
- [ ] 1 TL Dijon-Senf
- [ ] 1 EL Olivenöl
- [ ] In Schraubglas schütteln
- [ ] **Haltbarkeit:** 7 Tage

### Task 3B: Rotkohl marinieren (für Wraps & Salate) (10 Min)
- [ ] 60g fein gehobelter Rotkohl in Schüssel
- [ ] Marinade: 2 EL Apfelessig, 1 TL Ahornsirup, Zitronensaft, Kreuzkümmel
- [ ] Rotkohl kräftig massieren
- [ ] In Container, mindestens 1h ziehen lassen
- [ ] **Verwendung:** Tag 4 Mittag (Wrap)

### Task 3C: Pilz-Nuss-Füllung (für Wrap Tag 4) (15 Min)
- [ ] 150g gemischte Pilze klein hacken
- [ ] Trocken anbraten bis Wasser verdampft (5 Min)
- [ ] 1 TL Olivenöl, 1 Knoblauchzehe hinzufügen
- [ ] 20g Walnüsse (fein gehackt), 15g Cashews hinzufügen
- [ ] 1,5 EL Sojasauce, 1/2 TL Ahornsirup
- [ ] Gewürze: Paprika geräuchert, Thymian
- [ ] 5-7 Min braten bis goldbraun
- [ ] Abkühlen lassen, in Container
- [ ] **Haltbarkeit:** 4-5 Tage

### Task 3D: Tofu vorbereiten (ESSENTIELL für Protein-Optimierung!) ✨
- [ ] **870g Tofu** aus Packung nehmen (ca. 4 Blöcke à 200g)
- [ ] **Pressen:** 15-20 Min zwischen Küchentüchern, mit Buch/Brett beschwert
- [ ] In Würfel schneiden (2x2cm)
- [ ] In 5 Portionen aufteilen:
  - **180g** für Tag 1 Abend (mariniert mit Sojasauce)
  - **120g** für Tag 2 Mittag (Curry)
  - **150g** für Tag 3 Mittag (bereits im Original-Rezept, Miso-Suppe)
  - **120g** für Tag 4 Abend (Buddha-Bowl, mariniert mit Sojasauce)
  - **150g** für Tag 5 Mittag (Curry)
  - Rest: Puffer
- [ ] **Marinade vorbereiten:** 4 EL Sojasauce, 1 TL Ingwer, 1 Knoblauchzehe
- [ ] Tofu-Portionen in separaten Containern mit/ohne Marinade lagern
- [ ] **Haltbarkeit:** 4-5 Tage im Kühlschrank

---

## 📦 PHASE 4: PORTIONIEREN & LAGERN
⏰ **11:30 - 12:15 (45 Min)**

### Frühstück (bereits erledigt):
- [x] 4 Overnight Oats im Kühlschrank
- [ ] Toppings separat lagern:
  - Tag 1: 1 Apfel, Walnüsse, Zimt (am Morgen frisch schneiden)
  - Tag 2: Kürbis-/Sonnenblumenkerne, TK-Beeren (in Glas am Vorabend)
  - Tag 3: 1/2 Banane, Mandeln, Nussmus (am Morgen frisch)
  - Tag 4: Quinoa-Bowl (Sonntag Abend 60g Quinoa kochen für Mo Früh)
  - Tag 5: TK-Beeren, Hanfsamen (in Glas am Vorabend)

### Mittagessen portionieren:

#### Container 1 - Tag 1 Mittag (Buddha-Bowl):
- [ ] 80g gekochte Quinoa (Basis)
- [ ] 120g geröstete Kichererbsen
- [ ] 80g gedämpfter Brokkoli
- [ ] 100g gerösteter Hokkaido-Kürbis
- [ ] Label: "Mo Mittag - Buddha-Bowl" + Tahini separat + 1/4 Avocado frisch

#### Container 2 - Tag 2 Mittag (Rotkohl-Curry mit Tofu):
- [ ] **Am Sonntag nicht komplett vorbereiten!**
- [ ] Komponenten vorbereitet:
  - 150g geschnittener Rotkohl (in Beutel)
  - 150g gekochte Kichererbsen (in Container)
  - **120g Tofu-Würfel** (in Container) ✨
  - 80g Quinoa (in Container)
  - Curry-Gewürze abmessen (in kleinem Beutel)
- [ ] **Montag Morgen/Abend:** 20 Min Curry kochen - Tofu zuerst anbraten! ✨

#### Container 3 - Tag 3 Mittag (Miso-Suppe):
- [ ] **Nicht komplett vorkochen!**
- [ ] Komponenten:
  - 150g geschnittener Lauch (in Ringe, gut gewaschen)
  - 100g Shiitake-Pilze (in Scheiben, in Beutel)
  - 150g Tofu-Würfel
  - Miso-Paste (separat!)
- [ ] **Dienstag Morgen:** 15 Min Suppe kochen (Miso erst am Ende!)

#### Container 4 - Tag 4 Mittag (Wrap):
- [ ] Wrap-Komponenten separat:
  - Pilz-Nuss-Füllung (Container)
  - Marinierter Rotkohl (Container)
  - Hummus (50g)
  - Rucola (frisch in Beutel)
  - Tahini-Dressing
  - Wrap (separat)
- [ ] **Assembly:** Am Morgen oder direkt vor Essen

#### Container 5 - Tag 5 Mittag (Kichererbsen-Curry mit Tofu):
- [ ] Siehe Tag 2 - ähnliche Strategie
- [ ] Komponenten: 150g Kichererbsen, **150g Tofu**, Pilze, Spinat, Gewürze ✨
- [ ] Curry frisch kochen (20 Min)

### Abendessen portionieren:

#### Container 6 - Tag 1 Abend (Rotkohl-Apfel-Salat mit Tofu):
- [ ] 120g fein gehobelter Rotkohl
- [ ] 120g Cannellini-Bohnen
- [ ] **180g marinierter Tofu** (angebraten oder roh mariniert) ✨
- [ ] Dressing separat (kleiner Container)
- [ ] **Am Abend frisch:** Apfel schneiden, Walnüsse, Rucola hinzufügen
- [ ] **Wenn roh:** Tofu 5-7 Min anbraten vor Servieren

#### Container 7 - Tag 2 Abend (Linsen-Salat, Protein-optimiert):
- [ ] **180g gekochte Linsen** (erhöht!) ✨
- [ ] **Am Abend frisch:** Tomaten, Gurke, Rucola, Dressing

#### Container 8 - Tag 3 Abend (Gerösteter Rotkohl-Salat, Protein-optimiert):
- [ ] Gerösteter Rotkohl (Container)
- [ ] **300g geröstete Kichererbsen** (erhöht!) ✨
- [ ] 80g Quinoa
- [ ] Walnüsse, Rucola (frisch hinzufügen)

#### Container 9 - Tag 4 Abend (Buddha-Bowl mit Tofu):
- [ ] 120g Linsen
- [ ] **120g marinierter Tofu** (separat rösten am Tag, oder vorbereiten) ✨
- [ ] 80g Quinoa
- [ ] Geröstete Karotten
- [ ] Geröstete Rote Bete
- [ ] Gerösteter Fenchel
- [ ] Tahini, Hanfsamen, Kürbiskerne separat

#### Container 10 - Tag 5 Abend (Linsen-Salat, Protein-optimiert):
- [ ] **180g Linsen** (erhöht!) ✨
- [ ] 80g Quinoa/Buchweizen
- [ ] 120g gerösteter Hokkaido-Kürbis
- [ ] **Am Abend:** Apfel, Walnüsse, Feldsalat, Dressing

### Beschriftung & Lagerung:
- [ ] Alle Container mit Tag, Mahlzeit, Datum beschriften
- [ ] Kühlschrank organisieren:
  - **Oberste Reihe:** Overnight Oats (Gläser)
  - **Mittlere Reihe:** Mittagessen (Container 1-5)
  - **Unterste Reihe:** Abendessen (Container 6-10)
  - **Tür:** Dressings, Nussmus, Toppings
- [ ] Basis-Komponenten extra:
  - Restliche Quinoa (großer Container)
  - Restliche Linsen (Container)
  - Restliche Kichererbsen (Container)

---

## 🧹 PHASE 5: AUFRÄUMEN & QUALITY CHECK
⏰ **12:15 - 12:30 (15 Min)**

### Aufräumen:
- [ ] Töpfe und Backbleche reinigen
- [ ] Arbeitsflächen wischen
- [ ] Spülmaschine starten

### Quality Check:
- [ ] Alle 15 Mahlzeiten vorbereitet? ✅
- [ ] Overnight Oats im Kühlschrank? ✅
- [ ] Dressings beschriftet? ✅
- [ ] Alle Container verschlossen? ✅
- [ ] Haltbarkeit kontrolliert? ✅

### Einkaufsliste für Mittwoch:
- [ ] Frische Kräuter
- [ ] Rucola (falls nötig)
- [ ] Frisches Obst

---

## 📅 TÄGLICHER AUFWAND

### Montag - Freitag (täglich 5-15 Min)

**Morgen (5 Min):**
- Overnight Oats aus Kühlschrank + Toppings hinzufügen
- Oder Quinoa-Bowl aufwärmen (Tag 4)

**Mittag (0-15 Min):**
- Buddha-Bowls: Nur aufwärmen (2 Min Mikro)
- Currys: Frisch kochen (15 Min) - schmeckt besser!
- Wrap: Zusammenstellen (5 Min)
- Suppe: Frisch kochen (15 Min)

**Abend (5-10 Min):**
- Salate: Frische Komponenten hinzufügen
- Bowls: Aufwärmen oder kalt essen
- Dressing hinzufügen

---

## 🔄 MITTWOCH CHECK (20. November, 15 Min)

**Halbzeit-Kontrolle:**
- [ ] Neue Overnight Oats vorbereiten (für Do + Fr)
- [ ] Frische Zutaten checken
- [ ] Rucola/Kräuter auffüllen
- [ ] Quinoa nachkochen falls nötig
- [ ] Dressings aufschütteln

---

## 💡 PROFI-TIPPS

### Zeit sparen:
1. **Parallel kochen:** Alle Töpfe gleichzeitig nutzen
2. **Ofen auslasten:** 3 Bleche gleichzeitig (Rotkohl, Wurzelgemüse, Kichererbsen)
3. **Musik/Podcast:** Macht es angenehmer
4. **Helfende Hände:** Zu zweit geht's schneller

### Qualität bewahren:
1. **Frische separat:** Rucola, Kräuter, Toppings erst beim Essen hinzufügen
2. **Dressings separat:** Salate nicht vorher anmachen
3. **Knuspriges nachrösten:** Kichererbsen evtl. nochmal 5 Min im Ofen auffrischen

### Flexibilität:
1. **Tausche Tage:** Container sind flexibel einsetzbar
2. **Einfrieren:** Überschüssige Linsen/Kichererbsen portionsweise einfrieren
3. **Reste verwerten:** Basis-Komponenten für weitere Bowls nutzen

---

## 🎯 ALTERNATIVE: 2-TAGES-STRATEGIE

**Wenn 4h am Stück zu lang sind:**

### Samstag (2 Std):
- Phase 1: Getreide & Hülsenfrüchte kochen
- Phase 2: Gemüse rösten
- Overnight Oats vorbereiten

### Sonntag (2 Std):
- Phase 3: Spezial-Komponenten
- Phase 4: Portionieren
- Phase 5: Quality Check

---

## ✅ ERFOLGS-CHECKLISTE

Nach Meal Prep hast du:
- [ ] 4 Overnight Oats (Mo, Di, Mi, Fr)
- [ ] 1 Quinoa-Frühstück vorbereitet (Do)
- [ ] 5 Mittagessen (teils komplett, teils Komponenten)
- [ ] 5 Abendessen (teils komplett, teils Komponenten)
- [ ] 2 Dressings (Tahini, Apfel-Balsamico)
- [ ] Alle Basis-Komponenten (Quinoa, Linsen, Kichererbsen)
- [ ] Geröstetes Gemüse für ganze Woche

**Ergebnis:** 15 gesunde, nahrhafte Mahlzeiten für die ganze Woche! 🎉

---

## 📝 NOTIZEN & ANPASSUNGEN

**Nach dem ersten Meal Prep:**
- Was hat gut funktioniert?
- Was würdest du anders machen?
- Welche Gerichte waren deine Favoriten?
- Zeitplan realistisch?

**Für nächste Woche:**
- Rezepte wiederholen oder variieren?
- Andere Gewürzmischungen ausprobieren?
- Mehr/weniger Portionen?

---

**Viel Erfolg beim Meal Prep! 🌱💪**
